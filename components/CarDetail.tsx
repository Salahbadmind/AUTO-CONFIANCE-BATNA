...
import { 
  ArrowRight, Phone, MessageCircle, Clock, MapPin, Fuel, Gauge, 
  Calendar, Check, ShieldCheck, Share2, Maximize2, X, ChevronLeft, ChevronRight,
  FileText, Download, ExternalLink, Crown, UserCheck
} from 'lucide-react';
...
const CarDetail: React.FC<CarDetailProps> = ({
  ...
  const whatsappLink = `https://wa.me/${car.whatsapp.replace(/\D/g, '')}?text=${encodeURIComponent(
    `${t.whatsappMessagePrefix} ${car.brand} ${car.model} (${car.year}) - ${formatDzd(car.priceDzd)} - الموقع: ${car.location === 'algeria' ? t.badgeInAlgeria : t.badgeInChina}`
  )}`;

  const similarCars = allCars
    .filter(c => c.id !== car.id && (c.brand === car.brand || c.location === car.location))
    .slice(0, 3);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: `${car.brand} ${car.model} - Auto Elite`,
        text: `شاهد سيارة ${car.brand} ${car.model} في معرض Auto Elite الجزائر`,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="relative h-[380px] sm:h-[480px] bg-gray-950 rounded-2xl overflow-hidden border border-gray-800 group shadow-2xl">
            ...
            <div className="absolute top-4 right-4 z-10">
              {car.location === 'algeria' ? (
                <span className="bg-emerald-500 text-gray-950 text-xs font-black px-4 py-2 rounded-full shadow-xl flex items-center gap-1.5 border border-emerald-400">
                  <span className="w-2 h-2 rounded-full bg-gray-950 animate-pulse" />
                  <span>{t.badgeInAlgeria} - تسليم فوري</span>
                </span>
              ) : (
                <span className="bg-blue-600 text-white text-xs font-bold px-4 py-2 rounded-full shadow-xl flex items-center gap-1.5 border border-blue-400">
                  <Clock className="w-4 h-4" />
                  <span>{t.badgeInChina} - الشحن {car.shippingTime}</span>
                </span>
              )}
            </div>
        ...
        <div className="lg:col-span-5 flex flex-col justify-between bg-gray-900 border border-gray-800 rounded-2xl p-6 sm:p-8 shadow-2xl">
          ...
          <div>
              ...
              <div className="flex flex-col gap-3 mt-4">
                {car.location === 'rental' ? (
                  <div className="space-y-3">
                    ...
                    {/* Driver Option Banner & Rates */}
                    {car.driverOption === 'with_driver_only' ? (
                      <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3.5 text-right">
                        <div className="flex items-center gap-2 text-amber-400 font-black text-sm mb-1">
                          <Crown className="w-4 h-4 text-amber-400 shrink-0" />
                          <span>إجباري مع سائق خاص من المعرض</span>
                        </div>
                        <p className="text-[11px] text-gray-300 mb-2 leading-relaxed">
                          نظراً لفخامة هذه السيارة، يتم توفير سائق خاص ومحترف من المعرض لتأمين أعلى مستويات الراحة والأمان.
                        </p>
                        <div className="flex items-baseline gap-2 pt-2 border-t border-amber-500/20">
                          <span className="text-3xl font-black text-amber-400">
                            {formatDzd(car.dailyRateWithDriverDzd || car.dailyRateDzd || 0)}
                          </span>
                          <span className="text-xs text-gray-400">/ يوم (شامل السائق والوقود)</span>
                        </div>
                      </div>
                      ...
                      <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl p-3.5 space-y-2.5">
                        <div className="flex items-center gap-2 text-orange-400 font-bold text-xs">
                          ...
                          <span>متاحة بالخيارين (مع سائق خاص أو بدون سائق):</span>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <div className="bg-gray-950 p-2.5 rounded-lg border border-gray-800">
                            <span className="text-[10px] text-gray-400 block mb-0.5">🚗 بدون سائق (قيادة ذاتية):</span>
                            <span className="text-xl font-black text-orange-400">{formatDzd(car.dailyRateDzd || 0)}</span>
                            <span className="text-[10px] text-gray-400"> / يوم</span>
                          </div>
                          <div className="bg-gray-950 p-2.5 rounded-lg border border-amber-500/30">
                            <span className="text-[10px] text-amber-400 font-bold block mb-0.5 flex items-center gap-1">
                              <Crown className="w-3 h-3 text-amber-400" />
                              <span>مع سائق خاص من المعرض:</span>
                            </span>
                            <span className="text-xl font-black text-amber-300">{formatDzd(car.dailyRateWithDriverDzd || 0)}</span>
                            <span className="text-[10px] text-gray-400"> / يوم</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-950 border border-gray-800 rounded-xl p-3.5">
                        <div className="text-xs text-gray-400 mb-1 flex items-center gap-1.5">
                          <span>🚗 كراء قيادة ذاتية (بدون سائق)</span>
                        </div>
                        <div className="flex items-baseline gap-2">
                          <span className="text-3xl font-black text-orange-400">
                            {formatDzd(car.dailyRateDzd || car.priceDzd || 0)}
                          </span>
                          <span className="text-xs text-gray-400">/ يوم (سعر الكراء اليومي)</span>
                        </div>
                      </div>
                    )}

                    {/* Deposit & Minimum Days Pills */}
                    <div className="flex flex-wrap gap-2">
                      {car.securityDepositDzd && (
                        <div className="text-xs text-gray-300 bg-gray-950 px-3 py-1.5 rounded-lg border border-gray-800">
                          مبلغ التأمين (الكفالة): <span className="font-bold text-orange-400">{formatDzd(car.securityDepositDzd)}</span>
                        </div>
                      )}
                      {car.minRentalDays && (
                        <div className="text-xs text-gray-300 bg-gray-950 px-3 py-1.5 rounded-lg border border-gray-800">
                          أقل مدة كراء: <span className="font-bold text-white">{car.minRentalDays} أيام</span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    {car.priceDzd > 0 && (
                      <span className="text-xs text-gray-400">تسليم فوري شامل الوثائق</span>
                    )}
                  </div>
                )}
              </div>
            ...
            {/* Direct Contact Buttons (PROMINENT - NO CART) */}
            <div className="bg-gray-950 p-4 rounded-xl border border-gray-800 mb-6">
              ...
              <div className="mt-3 text-center text-[11px] text-gray-400 flex items-center justify-center gap-1">
                ...
                <span>ضمان المعاملات القانونية والتنقل للمعينة بالمعرض</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6 bg-gray-950 p-4 rounded-xl border border-gray-800/80 text-sm">
              ...
              <div className="flex items-center gap-2 text-gray-300">
                ...
                <div>
                  <span className="text-[11px] text-gray-500 block">{t.locationLabel}</span>
                  <span className="font-bold">{car.location === 'algeria' ? 'للبيع (تسليم فوري)' : 'للكراء (إيجار)'}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-gray-300 col-span-2 sm:col-span-1 pt-2 border-t border-gray-800">
                ...
                <div>
                  ...
                  <span className="font-bold text-amber-300">{car.interiorColor || 'جلد بني فخم / Cuir Marron'}</span>
                </div>
              </div>
            </div>
